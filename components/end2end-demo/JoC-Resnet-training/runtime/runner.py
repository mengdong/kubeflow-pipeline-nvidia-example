...
class Runner(object):
  ...
  def __init__(
    ...
    self._model = resnet_v1_5.ResnetModel(
      ...
      model_dir=run_config_additional.model_dir,
  ...
  def train(
    ...
    trt_graph = trt.create_inference_graph(
      ...
      outputs=None,
      input_saved_model_dir=os.path.join(self.run_hparams.model_dir,
    ...
    print("number of trt engine nodes: %d" % len([1 for n in trt_graph.node if str(n.op) == 'TRTEngineOp']))
    ...
    print("write graph to: %s/model.graphdef" % (self.run_hparams.model_dir))

    # model_copy_command = ['gsutil', '-m', 'cp', self.run_hparams.model_dir + '/model.graphdef',
    #                       self.run_hparams.storage_bucket_dir + '/' + args.model_version]
  ...
  def evaluate(
    ...
    num_epochs = num_iter
    num_samples = 10000
    num_steps = (num_samples // batch_size) * num_iter
    steps_per_epoch = num_steps / num_epochs
    num_decay_steps = num_steps
