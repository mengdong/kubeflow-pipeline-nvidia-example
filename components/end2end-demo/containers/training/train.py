...
CONT_TRTIS_RESOURCE_PATH = '/trtis_resource'
external_trtis_resource_path = args.output_dir
if args.model_version.strip() == '1':
  model_copy_command = ['gsutil', '-m', 'cp', CONT_TRTIS_RESOURCE_PATH,
                      args.output_dir]
...
# Save Keras model
tmp_model_path = os.path.join(args.ckpt_dir, "tmp")
...
tf_model_path = os.path.join(args.ckpt_dir, "tf_saved_model")
...
model_auth_command = ['gcloud', 'auth', 'activate-service-account', '--key-file',
                      '/secret/gcp-credentials/user-nvidia-sa.json']
result = subprocess.call(model_auth_command)
print(result)

model_copy_command = ['gsutil', '-m', 'cp', args.ckpt_dir + '/model.graphdef',
                      args.output_dir + '/' + args.model_version + '/model.graphdef']

result = subprocess.call(model_copy_command)
print(result)

print('input_dir: {}'.format(args.input_dir))
print('ckpt_dir: {}'.format(args.ckpt_dir))
